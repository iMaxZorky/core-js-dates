...
function formatDate(date) {
  ...
  return new Date(new Date(date).getTime() + offset).toLocaleString('en-US');
}
...
function getWeekNumberByDate(date) {
  ...
  const start = new Date(firstDateOfTheYear);
  const end = new Date(date);
  const millisecondsByDay = 86400000;
  const shift = firstDateOfTheYear.getDay() === 7 ? 1 : 0;

  start.setDate(firstDateOfTheYear.getDate() + shift);
  end.setDate(date.getDate() + ((-date.getDay() + 7) % 7));

  const difference = Math.floor((end - start) / millisecondsByDay);
...
function getNextFridayThe13th(date) {
  ...
  const shift = 5 - mutableDate.getDay();

  if (shift < 0) {
    mutableDate.setDate(mutableDate.getDate() + (7 + shift));
  } else {
    mutableDate.setDate(mutableDate.getDate() + shift);
  }
  while (mutableDate.getDate() !== 13) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const mutableDate = new Date(period.start.split('-').reverse().join('-'));
  const endDate = new Date(period.end.split('-').reverse().join('-'));
  const result = [];

  while (Date.parse(endDate) >= Date.parse(mutableDate)) {
    if (Date.parse(endDate) < Date.parse(mutableDate)) {
      break;
    }
    let count = countWorkDays;
    while (count > 0) {
      result.push(mutableDate.toLocaleDateString('en-GB').replaceAll('/', '-'));
    ...
    mutableDate.setDate(mutableDate.getDate() + countOffDays);
  }
  return result;
}
